...
#as constantes do modelo refletem a média de pessoas que as pessoas conversam vezes a probabilidade de passar a doença
...
def plotar(row):
    ...
    ax1.set_ylabel("Acumulado do nº de casos", family="arial", fontsize=12)
    ax1.legend(loc="best")
    ...
    fig.tight_layout()
    fig.savefig("COVID1403brasil.jpg", format="jpg", dpi=1000)
    plt.title("COVID 19 em 14/03 no Brasil")
    plt.grid(True)
    plt.show()
'''    
def datasetBuilder(linhas):
    #tirar o nome antes: how to handle the NaN object?
    dias = np.arange(1, len(linhas) + 1 , dtype=int)
    data = np.array([dias, linhas])
    print(data)
    #br = br.astype(int)
def main():
    coronga = pd.read_csv('./data/coronaMundo.csv')
    #for i in coronga:
    #br = np.delete(np.array(coronga.loc[0].array), 0,0)
    #print(br) 
    
    local = np.array(coronga.loc[0].array)
    
    #if len(data[0]) == len(data[1]):
        #print("deu boa dms")
    
    datasetBuilder(local)
